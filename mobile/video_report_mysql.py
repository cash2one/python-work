...
from report.AdMobile import AdReadFile, ReportOracleStock
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        ...
        print '''
         ...
         -DwebConsole	        eg: http://10.10.79.193:5000/
    ...
    jsonParallel = ReportJsonParallel(monitorReport,cpu=16)
    ...
    if cmp(sys.argv[1], '-Dsave') == 0:
        ...
        logdir = "/opt/video_report_yyyymm/mobile/"+monitorReport.mydate.get_now()+"/"
        error_log = logdir + 'stdout.log'
        ...
        jsonParallel.urls.append('http://api.my.tv.sohu.com/video/videoinfolist.do?src=pgcFc&vid=%s')
        ...
        logdir = "/opt/video_report_yyyymm/mobile/"+monitorReport.mydate.get_now()+"/"
        oracle_log = logdir + 'ORACLE_STOCK_SQL.log'
        oracleWrap = ReportOracleStock(monitorReport,ReportOracle.conn_oracle_params)
        oracleWrap.write_rows_file(oracle_log)

        adfileRead.read_file(oracle_log)
        mysqlWrap.check_create_table(26)
